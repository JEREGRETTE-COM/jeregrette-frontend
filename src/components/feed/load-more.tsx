...
export function LoadMore({
  ...
  const [ending, setEnding] = useState<"end" | "stuck">("end");
  const [pending, startTransition] = useTransition();
  const seen = useRef(new Set(initialIds));

  function loadMore() {
    startTransition(async () => {
      const page = await loadMoreFeedAction(cursor, [...seen.current]);

      const fresh = page.items.filter((item) => !seen.current.has(itemId(item)));
      for (const item of fresh) seen.current.add(itemId(item));

      if (fresh.length === 0) {
        // Nothing new while the API still claims more: pagination is broken there.
        setEnding(page.hasMore ? "stuck" : "end");
        setHasMore(false);
        return;
      }
  ...
  return (
    <>
      {items.map((item) =>
        item.kind === "regret" ? (
          <RegretCard key={item.regret.id} regret={item.regret} currentUserId={currentUserId} />
        ) : (
          <RepostCard key={item.repost.id} repost={item.repost} currentUserId={currentUserId} />
        ),
      )}
      ...
      {hasMore ? (
        ...
        <p className="text-muted mt-[10px] pb-[10px] text-center text-[14px]">
          Impossible de charger la suite pour le moment.
        </p>
      ) : (
        <p className="text-muted mt-[10px] pb-[10px] text-center text-[14px]">
